...
既存 sandbox へ後付けで SSH を入れるフローです。sandbox の作成自体はこの repo の責務にせず、`sbx run shell ...` などで作られた sandbox に対して `sbx-vscode-include.sh` で SSH kit を導入し、`sbx-vscode-connect.sh` で起動と VS Code 接続を行います。
...
```bash
./sbx-vscode-include.sh my-sandbox
```

3. VS Code で接続する

```bash
./sbx-vscode-connect.sh
```

`sbx-vscode-include.sh` は次を行います。
...
`sbx-sshd` では OpenSSH の導入や host key 作成も kit の `startup` に寄せてあります。`sbx kit add` の適用対象として `startup` は含まれる一方、`install` は help 上では明示されていないため、`v4` では `install` に依存しない構成にしています。

`sbx-vscode-connect.sh` は `.sbx_remote_ssh` を読み、必要なら sandbox を起動し、host port を publish してから VS Code Remote SSH を開きます。
...
```bash
PUBLIC_KEY_PATH="$HOME/.ssh/id_rsa.pub" ./sbx-vscode-include.sh my-sandbox
```

## 変更できる環境変数

`sbx-vscode-include.sh`:
